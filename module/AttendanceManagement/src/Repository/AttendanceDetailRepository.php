...
//        print_r($statement->getSql()); die();
    ...
    public function fetchEmployeeShfitDetails($employeeId) {
        $sql = "SELECT INITCAP(TO_CHAR(SYSDATE, 'HH:MI AM')) AS CURRENT_TIME,
            INITCAP(TO_CHAR(S.START_TIME, 'HH:MI AM')) AS CHECKIN_TIME, 
            INITCAP(TO_CHAR(S.END_TIME, 'HH:MI AM')) AS CHECKOUT_TIME,
            INITCAP(TO_CHAR(S.HALF_DAY_END_TIME, 'HH:MI AM')) AS HALF_DAY_CHECKOUT_TIME,
            ESA.*,S.* FROM HRIS_EMPLOYEES E
                ...
                WHERE E.EMPLOYEE_ID=$employeeId AND ESA.STATUS='E' AND S.STATUS = 'E' AND ESA.MODIFIED_DT IS NULL
    ...
    public function fetchEmployeeDefaultShift() {
      ...
      S.*
            FROM HRIS_SHIFTS S
            WHERE S.DEFAULT_SHIFT='Y' AND S.STATUS = 'E'
            AND (TO_DATE(TRUNC(SYSDATE), 'DD-MON-YY') BETWEEN TO_DATE(S.START_DATE, 'DD-MON-YY') AND TO_DATE(S.END_DATE, 'DD-MON-YY'))";
        $statement = $this->adapter->query($sql);
        $result = $statement->execute();
        return $result->current();
