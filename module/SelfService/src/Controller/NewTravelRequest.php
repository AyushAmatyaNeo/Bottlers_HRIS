...
class NewTravelRequest extends HrisController {
    ...
    public function expenseAddAction() {
        ...
        $expenseHeads = array(
            ...
            array('gl' => 'Cafeteria Expenses', 'name' => 'Cafeteria Expenses'),
            ...
            array('gl' => 'Guest House Expenses', 'name' => 'Guest House Expenses'),
            array('gl' => 'Hotel Meals (exclusive of Tips)', 'name' => 'Hotel Meals (exclusive of Tips)'),
            ...
            if ($postData['travelIdToInsert'] != '') {
                ...
                if ($postData['erTypeL'][0] != -1 && $postData['traveltype'] == 'LTR' &&  $postData['travelIdToInsert'] != ''){
                    ...
                    for ($i = 0; $i < count($postData['erTypeL']); $i++){
                        ...
                        $model->currency = 'NPR';
                        // echo '<pre>'; print_r(count($postData['erTypeI'])); die;
                        $repo->add($model);
                ...
                $reqModel->requestedAmount = $this->repository->getTotalExpenseAmount($travelNewId);
                $reqModel->referenceTravelId = $travelId;
                $reqModel->departureDate = $detail->departureDate;
                $reqModel->returnedDate = $detail->returnedDate;
                ...
                if ($postData['erTypeL'][0] != -1){
                    ...
                    $reqModel->requestedDate = Helper::getcurrentExpressionDate();
                    $reqModel->status = 'RQ';
                    $reqModel->destination = $postData['destination'][0];
                ...
                // echo '<pre>'; print_r('sdcsd'); die;
            }
            // echo '<pre>'; print_r('gfhf$postData'); die;
    ...
    public function editAction() {
        ...
        $detailxdc = $this->repository->fetchById($id);
        // echo '<pre>'; print_r($detailxdc); die;
        ...
        if ($request->isPost()) {
           ...
           if( isset($postData['erTypeL']) == true && $detailxdc['REFERENCE_TRAVEL_ID'] != ''){
            for ($i = 0; $i < count($postData['erTypeL']); $i++){
                // var_dump('hrhf'); die;
                //  echo '<pre>'; print_r($postData); die;
                $model->travelExpenseId = ((int) Helper::getMaxId($this->adapter, "hris_travel_expense", "TRAVEL_EXPENSE_ID")) + 1;
          ...
          elseif( isset($postData['erTypeI']) == true){
            ...
            // var_dump($totalamountfortravle); die;
