...
//
        ...
        $leaveReqNotiMod->actualDays = ($leaveApply->halfDay == 'F' || $leaveApply->halfDay == 'S') ? $leaveApply->noOfDays / 2 : $leaveApply->noOfDays;
...
//
    ...
    private static function travelApproveThird(TravelRequest $request, AdapterInterface $adapter, Url $url, string $status) {
        self::initFullModel(new TravelRequestRepository($adapter), $request, $request->travelId);
        $recommdAppRepo = new RecommendApproveRepository($adapter);
        // 163 is designation id of humanresource director
        $hrDirId = $recommdAppRepo->getHrDirId(163);
        ...
        switch ($request->requestedType) {
            case self::TRAVEL_ADVANCE_REQUEST:
                $notification->route = json_encode(["route" => "travelApprove", "action" => "view", "id" => $request->travelId, "role" => 'A2']);
                break;
            case self::TRAVEL_EXPENSE_REQUEST :
                $notification->route = json_encode(["route" => "travelApprove", "action" => "viewExpense", "id" => $request->travelId, "role" => 'A2']);
                break;
    ...
    private static function travelApproveFourth(TravelRequest $request, AdapterInterface $adapter, Url $url, string $status) {
        self::initFullModel(new TravelRequestRepository($adapter), $request, $request->travelId);
        $recommdAppRepo = new RecommendApproveRepository($adapter);
        // 93 is designation id of managing director
        $hrDirId = $recommdAppRepo->getHrDirId(93);
        $recommdAppModel = self::findRecApp($request->employeeId, $adapter);
        // var_dump($request->employeeId); die;

        $notification = self::initializeNotificationModel(
            $request->employeeId, $hrDirId[0]['EMPLOYEE_ID']  , \Notification\Model\TravelReqNotificationModel::class, $adapter);
    ...
    private static function travelApproveFifth(TravelRequest $request, AdapterInterface $adapter, Url $url, string $status) {
        self::initFullModel(new TravelRequestRepository($adapter), $request, $request->travelId);
        $recommdAppModel = self::findRecApp($request->employeeId, $adapter);
        $recommdAppRepo = new RecommendApproveRepository($adapter);
        // 166 is designation id of fiannce manager director
        $hrDirId = $recommdAppRepo->getHrDirId(166);
        $notification = self::initializeNotificationModel(
            $request->employeeId, $hrDirId[0]['EMPLOYEE_ID']  , \Notification\Model\TravelReqNotificationModel::class, $adapter);
        ...
        switch ($request->requestedType) {
            case self::TRAVEL_ADVANCE_REQUEST:
                $notification->route = json_encode(["route" => "travelApprove", "action" => "view", "id" => $request->travelId, "role" => 'A4']);
                break;
            case self::TRAVEL_EXPENSE_REQUEST :
                $notification->route = json_encode(["route" => "travelApprove", "action" => "viewExpense", "id" => $request->travelId, "role" => 'A4']);
                break;
            case self::TRAVEL_ADVANCE_INTER :
                $notification->route = json_encode(["route" => "travelApprove", "action" => "view", "id" => $request->travelId, "role" => 'A4']);
                break;
    ...
    private static function travelExpenseApprove(TravelRequest $request, AdapterInterface $adapter, Url $url, string $status) {
        self::initFullModel(new TravelRequestRepository($adapter), $request, $request->travelId);
        $recommdAppModel = self::findRecApp($request->employeeId, $adapter);
        // Finance Controller
        $recommdAppRepo = new RecommendApproveRepository($adapter);
        // 34 is designation id of fiannce manager director
        $hrDirId = $recommdAppRepo->getHrDirId(34);
        $notification = self::initializeNotificationModel(
        $request->employeeId, $hrDirId[0]['EMPLOYEE_ID'], \Notification\Model\TravelReqNotificationModel::class, $adapter);
        ...
        switch ($request->requestedType) {
            case self::TRAVEL_ADVANCE_REQUEST:
                ...
                break;
            case self::TRAVEL_EXPENSE_REQUEST :
                $notification->route = json_encode(["route" => "travelApprove", "action" => "expenseDetail", "id" => $request->travelId, "role"=>'RP']);
                break;
            case self::TRAVEL_ADVANCE_INTER :
                $notification->route = json_encode(["route" => "travelApprove", "action" => "view", "id" => $request->travelId]);
                break;
        ...
        $title = "Travel Expense Requested";
        $desc = "Travel Expense Requested";
    ...
    private static function travelApproveSixth(TravelRequest $request, AdapterInterface $adapter, Url $url, string $status) {
        self::initFullModel(new TravelRequestRepository($adapter), $request, $request->travelId);
        // $designation = '93';
...
//                ${"fn" . NotificationEvents::ADVANCE_CANCELLED}($model, $adapter, $url);
            ...
            case NotificationEvents::TRAVEL_EXPENSE_APPROVE_ACCEPTED:
                // var_dump('hern'); die;
                self::travelExpenseApprove($model, $adapter, $url, self::ACCEPTED);
                break;   
