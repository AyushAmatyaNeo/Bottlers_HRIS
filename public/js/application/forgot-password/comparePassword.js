(function ($, app) {
    ...
    $("#password").on("blur keyup", function () {
        ...
        var reg = /^(?=.*?[A-Z])(?=(.*[a-z]){1,})(?=(.*[\d]){1,})(?=(.*[\W]){1,})(?!.*\s).{12,}$/;

        if (!reg.test(passwordValue)) {
            $("#errorMsgPwd").html("The password should be at least 12 character long and should contain Numeric, Alphabet, Capital Letter, Symbol Combinations");
        }
        else {
            $("#errorMsgPwd").html("");
