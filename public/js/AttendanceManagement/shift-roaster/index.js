(function ($, app) {
    ...
    $(document).ready(function () {
		...
		columns.push({field: "D", title:"SHIFT NAME",with:120 });
    	app.initializeKendoGrid($table, columns);

		$("#submit").on('click', function(){
			if(prompt("Make sure all data are correct.Type CONFIRM to proceed.")!=="CONFIRM"){return;}
			app.serverRequest(document.updateShiftRoasterLink,{data:excelData,basedOn: basedOnFlag}).then(function(){
    	...
    	// $("#submit").on('click', function(){
			...
			// app.serverRequest(document.updateShiftRoasterLink, {data : excelData}).then(function(){
				//             app.showMessage('Operation successfull', 'success');
				//         }, function (error) {
				//             console.log(error);
		// console.log('fcfgd');
